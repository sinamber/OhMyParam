...
validator={
	...
	mobile : /^((\(\d{3}\))|(\d{3}\-))?1[38][0-9]\d{8}?$|15[0-35-9]\d{8}?$/,
	phoneMobile:/^(((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?)|(((\(\d{3}\))|(\d{3}\-))?1[38][0-9]\d{8}?$|15[0-35-9]\d{8}?)$/,
	url : /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	idCard : "this.isIdCard(value)",
	currency : /^\d+(\.\d+)?$/,
	number : /^\d+$/,
	zip : /^[1-9]\d{5}$/,
	ip  : /^(2[5][0-5]|2[0-4][0-9]|[1]{1}[0-9]{0,2}|[1-9]{1}[0-9]{1}|[1-9])\.(2[5][0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{0,2}|[1-9]{1}[0-9]{1}|[1-9])\.(2[5][0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{0,2}|[1-9]{1}[0-9]{1}|[1-9])\.(2[5][0-4]|2[0-4][0-9]|[1]{1}[0-9]{0,2}|[1-9]{1}[0-9]{1}|[1-9])$/, //^[\d\.]{7,15}$/,
	qq : /^[1-9]\d{4,8}$/,
	integer : /^[-\+]?\d+$/,
	double : /^[-\+]?\d+(\.\d+)?$/,
	wsdl : /^(http|https):\/\/[A-Za-z0-9]+\.?[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*(.wsdl)$/,
	english : /^[A-Za-z]+$/,
	chinese : /^[\u0391-\uFFE5]+$/,
	notchinese:/^[^\u0391-\uFFE5]+$/,
	...
	doajax : function(value) {	
		...
		var oldValue=this['element'].attr('oldValue');
		var arr_errmsg ;
		var errmsg ;
				...
				data = data.replace(/(^\s*)|(\s*$)/g, "");
				   if(data != 'success'){
					  ...
					  (type!='checkbox' && type!='radio' && element.addClass(validator.errorInput));
						if(msgid.length>0){
							msgid.html('<label class="btn-error js-error help-block col-xs-12 col-sm-reset inline"><span>&nbsp;<i class="fa fa-times-circle"></i>&nbsp;'+errmsg+'</span></label>');
						} else{
							jQuery('<label class="btn-error js-error help-block col-xs-12 col-sm-reset inline"><span>&nbsp;<i class="fa fa-times-circle"></i>&nbsp;'+errmsg+'</span></label>').insertAfter(element);
						}
						return false;
					}
					...
					if(data=='success') {
						element.parent('*').find('.js-error').remove();
						/*if(msgid.length>0){
							msgid.html('<span class="ico-success js-error"></span>');
					   } else {
							jQuery('<span class="ico-success js-error"></span>').insertAfter(element);
						}
						*/
						return true;
					}
...
validator.removeErr =  function(element){
	...
	element.parent('*').find('.js-error').remove();
	//jQuery('<span class="ico-ok js-error"></span>').insertAfter(element);
}
...
validator.check=function(obj){
	...
	//datatype 里面没有require 而且value有为空就不验证
	if(datatype.indexOf("require")==-1&&value==''){
	...
	jQuery.each(datatypes,function(index,type){
		...
		if(validator.checkDatatype(obj,type)==false){  //the form element validate failed
			...
			if(msgid.length>0) {
				...
				obj.parent('*').find('.ico-success,.js-error').remove();
				//jQuery('<span class="ico-success js-error"></span>').insertAfter(obj);
			}				
		}
	});
	return isValid;
...
jQuery.fn.checkForm = function(afterCheck){
